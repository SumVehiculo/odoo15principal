...
class product_template(models.Model):
	...
	check_unidad_especifica = fields.Boolean('Costeo x Unidad Especifica',default=False)


class stock_production_lot(models.Model):
	_inherit = 'stock.production.lot'

	precio_compra = fields.Float('Precio Compra',digits=(12,2))
	gasto_vinculado = fields.Float('Gasto Vinculado',digits=(12,2))
	precio_final = fields.Float('Precio Final',digits=(12,2))
		...
		for self in selfs:
			if 'nomore' in self.env.context:
				...
				if self.purchase_line_id.id and 'price_unit_it' not in vals and not self.origin_returned_move_id.id:
					...
					for elemx in self.move_line_ids:
						if elemx.lot_id.id:
							elemx.lot_id.precio_compra = elem * self.picking_id.tc
							elemx.lot_id.precio_final = elem * self.picking_id.tc + elemx.lot_id.gasto_vinculado
				elif self.origin_returned_move_id.id and 'price_unit_it' not in vals:					
					for elemx in self.move_line_ids:
						if elemx.lot_id.id:
							elemx.lot_id.precio_compra = self.origin_returned_move_id.price_unit_it * self.picking_id.tc
							elemx.lot_id.precio_final = self.origin_returned_move_id.price_unit_it * self.picking_id.tc + elemx.lot_id.gasto_vinculado
		return t
...
class make_kardex_valorado(models.TransientModel):
	...
	tipo_kardex = fields.Selection([('promedio','Costo Promedio'),('unidad','Unidades Especificas')],'Tipo de Costeo',default='promedio')
