...
class F2Register(models.Model):
	...
	si_debe = fields.Float(string='Debe Inicial')
	si_haber = fields.Float(string='Haber Inicial')
	...
	def init(self):
		...
		self.env.cr.execute('''
			...
			from get_sumas_mayor_f2('2019/01/01','2019/01/01',1,TRUE) a1
			left join account_account a2 on a2.id=a1.account_id
			order by a2.code)T limit 1
