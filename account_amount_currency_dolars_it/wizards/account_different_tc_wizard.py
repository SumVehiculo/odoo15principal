...
class AccountDifferentTcWizard(models.TransientModel):
	_name = 'account.different.tc.wizard'
	...
	def get_report(self):
		...
		self.env.cr.execute("""SELECT dg.periodo,aj.name as libro,dg.voucher,dg.fecha,dg.td_sunat,dg.nro_comprobante,dg.doc_partner as ruc, dg.partner,dg.cuenta, dg.balance as monto_mn,
		dg.importe_me as monto_me, dg.tc, (case when coalesce(dg.importe_me,1) <> 0 then round(dg.balance/coalesce(dg.importe_me,1),3) else 0 end) as tc_calculado
		FROM get_diariog_usd('%s','%s',%d,%s) dg
		LEFT JOIN  account_move am ON am.id = dg.move_id
		LEFT JOIN account_journal aj on aj.id = am.journal_id
		where dg.tc <> (case when coalesce(dg.importe_me,1) <> 0 then round(dg.balance/coalesce(dg.importe_me,1),3) else 0 end)
		...
		return self.env['popup.it'].get_file('TC diferentes.xlsx',output_datas)