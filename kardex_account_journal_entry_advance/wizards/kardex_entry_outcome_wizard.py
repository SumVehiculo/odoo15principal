...
class KardexEntryOutcomeWizard(models.TransientModel):
	_name = 'kardex.entry.outcome.wizard'
	...
	def make_invoice(self):
		...
		register = self.env['kardex.entry.outcome.it'].search([('period_id','=',self.period.id),('company_id','=',self.company_id.id)],limit=1)
		if register:
			...
			register = self.env['kardex.entry.outcome.it'].create({
		...
		for top in res:
			...
			move_id = self.env['account.move'].create({
				...
				'line_ids':lineas,
				'ref': 'SALIDAS-%s'%(self.period.code),
				'glosa': 'POR LOS SALIDAS EN ALMACEN MES DE %s'%(self.period.name),
				'kardex_outcome_id':register.id,
				'move_type':'entry'})
