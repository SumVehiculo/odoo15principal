...
class KardexEntryIncomeWizard(models.TransientModel):
	...
	def make_invoice(self):
		...
		for top in res:
			self.env.cr.execute("""select tipo_name,cta_debe, sum(round(coalesce(amount,0),2)) as debit from kardex_entry_income_book
			where tipo_op = '%s'
			...
			for elem in dic_debit:
				...
				lineas.append(vals)
			self.env.cr.execute("""select tipo_name,cta_haber,SUM(round(coalesce(amount,0),2)) as credit from kardex_entry_income_book
			where tipo_op = '%s'
