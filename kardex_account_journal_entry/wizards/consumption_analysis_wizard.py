...
class ConsumptionAnalysisWizard(models.TransientModel):
	...
	def get_excel(self):
		...
		for line in dic:
			...
			worksheet.write(x,6,line.analytic_tag_id.name if line.analytic_tag_id else '',formats['especial1'])
			x += 1
		...
		return self.env['popup.it'].get_file(u'Análisis de Consumo.xlsx',base64.encodebytes(b''.join(f.readlines())))
