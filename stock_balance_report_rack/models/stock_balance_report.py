...
class StockBalanceReport(models.Model):
	...
	def get_balance_view(self):
		...
		self.env.cr.execute("""
			...
			left join product_product pp on pp.id = vstf.p_id
			left join (
				select ubi.name ubiname, ubi.posicion ubiposic, ubi.product_id as ubiprod, ubi.almacen as ubialm
				from ubicacion ubi
			)ubii on ubii.ubiprod = pp.product_tmpl_id and ubii.ubialm = vstf.alm_id
			...
			group by
			producto,cod_pro,categoria_id, p_id, alm_id, ubii.ubiname, ubii.ubiposic
...
class StockBalanceReportLote(models.Model):
	...
	def get_balance_view(self):
		...
		self.env.cr.execute("""
			...
			and vstf.estado = 'done'
			group by
			producto,cod_pro,categoria_id, p_id, alm_id,lote,lote_id,ubii.ubiname, ubii.ubiposic
			having coalesce(sum(coalesce(vstf.entrada,0)),0) -  coalesce(sum(coalesce(vstf.salida,0)),0) != 0
			;
		""")
