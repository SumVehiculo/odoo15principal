...
class QueryDeluxe(models.Model):
	...
	def get_credentials(self):
		wizard = self.env['validation.sql.wizard'].create({
			'querydeluxe_id': self.id
		})
		module = __name__.split('addons.')[1].split('.')[0]
		view = self.env.ref('%s.view_validation_sql_wizard_form' % module)
		return {
			'name':u'Ingresar PIN',
			'res_id':wizard.id,
			'view_mode': 'form',
			'res_model': 'validation.sql.wizard',
			'view_id': view.id,
