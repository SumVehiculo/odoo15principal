	...
	def get_vacation_employee(self,employee,show_all):
		...
		for employee in employes:
			...
			if saldos:
				...
				date_aplication = saldos.date_aplication
				...
				date_time_obj = datetime.strptime(date_time_obj, '%Y-%m-%d %H:%M:%S').date()
			...
			# date_time_actual = datetime.strptime(date_time_actual, '%Y-%m-%d').date()
				...
				date_aplication = contrato_v.date_start
				date_time_obj = contrato_v.date_start
			...
			# print("fecha_saldo",fecha_saldo)
			af = 102.5 if last_contract.employee_id.children > 0 else 0
			amount_contrato = last_contract.wage + af
			...
			for cuenta in range(conteo):
				...
				if act_date >= fecha_saldo:
					...
					if (act_date + relativedelta(months=12)).year == date_time_end_obj.year:
						...
						# print("date_aplication",date_aplication)
						if (date_time_end_obj + relativedelta(months=12) - timedelta(days=1)) > date_aplication:
							if act_date >= (date_time_end_obj + relativedelta(months=12) - timedelta(days=1)):
								vals = {
									'employee_id': employee.id,
									'date_aplication': date_time_end_obj + relativedelta(months=12) - timedelta(days=1),
									...
									'days': 30,
									'days_rest': 30,
									'year': str(date_time_end_obj.year),
